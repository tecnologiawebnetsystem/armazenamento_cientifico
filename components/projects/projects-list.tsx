...
export function ProjectsList({ canCreate }: { canCreate: boolean }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="overflow-hidden rounded-2xl border border-petrobras-blue/20 bg-card p-6 shadow-lg shadow-petrobras-blue/8 ring-1 ring-petrobras-green/20"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-mono text-xs font-semibold uppercase tracking-[0.2em] text-petrobras-green">Gestão científica</p><h1 className="mt-1 text-3xl font-semibold tracking-tight text-petrobras-blue">Projetos</h1><p className="mt-2 max-w-2xl text-sm text-muted-foreground">Acompanhe projetos, áreas responsáveis, acessos e armazenamento em um só lugar.</p></div>{canCreate && <Button render={<Link href="/projetos/novo" />} nativeButton={false} className="bg-petrobras-green text-primary-foreground shadow-sm shadow-petrobras-green/20 hover:-translate-y-0.5 hover:bg-petrobras-green/90 hover:shadow-md"><FolderPlusIcon data-icon="inline-start" />Novo projeto</Button>}</div></div>
