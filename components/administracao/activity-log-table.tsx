...
import { AlertCircleIcon, CheckCircle2Icon, ChevronLeftIcon, ChevronRightIcon, DownloadIcon, HistoryIcon, SearchIcon } from "lucide-react"
...
import { ExportFieldsDialog, type ExportField } from "@/components/export-fields-dialog"
...
export function ActivityLogTable() {
  ...
  const exportLogs = (fields: string[]) => { const next = new URLSearchParams(searchParams.toString()); next.set("format", exportFormat); next.set("fields", fields.join(",")); window.location.href = `/api/activity-logs?${next}` }
  const options = useMemo(() => ({ actions: [...new Set(logs.map((log) => log.acao))] }), [logs])
  return <Card>
    <CardHeader className="gap-4"><div className="flex flex-wrap items-start justify-between gap-4"><div><CardTitle>Trilha de auditoria</CardTitle><CardDescription>{pagination.total} eventos no escopo autorizado. {isValidating ? "Atualizando…" : ""}</CardDescription></div><div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => { setExportFormat("txt"); setExportOpen(true) }}><DownloadIcon data-icon="inline-start" />TXT</Button><Button variant="outline" size="sm" onClick={() => { setExportFormat("csv"); setExportOpen(true) }}><DownloadIcon data-icon="inline-start" />CSV</Button></div></div>
    ...
    <ExportFieldsDialog open={exportOpen} onOpenChange={setExportOpen} format={exportFormat} title="logs de auditoria" fields={exportFields} onConfirm={exportLogs} />
  </Card>
}
