...
export function ActivityLogTable() {
  ...
  const exportLogs = (fields: string[], formats: ("csv" | "txt")[]) => { formats.forEach((format) => { const next = new URLSearchParams(searchParams.toString()); next.set("format", format); next.set("fields", fields.join(",")); window.open(`/api/activity-logs?${next}`, "_blank") }) }
