...
class PlatformRepository:
    ...
    async def context(self, session_user: dict[str, Any]) -> dict[str, Any]:
        ...
        if is_sponsor and not any(menu.get("rota") == "/projetos" for menu in menus):
            menus.append({"id": "menu-projetos", "nome": "Projetos", "rota": "/projetos", "icone": "folder", "ordem": 10, "parent_id": None})
    ...
    async def dashboard(self) -> dict[str, Any]:
        ...
        activity = await self.rows(f"select id, user_id as \"userId\", action as acao, entity as entidade, entity_id as \"entidadeId\", details as detalhes, created_at as \"criadoEm\", result as resultado, CASE WHEN entity = 'project' THEN entity_id ELSE NULL END as \"projetoId\" from {self.schema}.activity_logs order by created_at desc limit 10")
