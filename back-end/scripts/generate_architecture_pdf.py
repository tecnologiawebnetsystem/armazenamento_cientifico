...
def main():
    ...
    story = [
        ...
        Spacer(1, 4 * mm),
        table([["Controle do documento", "Valor"], ["Versão", "2.2"], ["Status", "Para validação do gestor e arquitetura"], ["Escopo", "Primeira fase do SIGAC"], ["Autoridade de identidade", "CAV4 — grupos, papéis e permissões"], ["Persistência", "Aurora PostgreSQL"], ["Stack", "Next.js · FastAPI · SQLAlchemy · Alembic"]], [52 * mm, 126 * mm], s),
        Spacer(1, 8 * mm),
        ...
        table([["Perfil", "Responsabilidade e acesso esperado"], ["Administrador", "Acesso total ao SIGAC: projetos, perfis, menus, permissões locais, relatórios e auditoria; não altera permissões físicas de pastas"], ["Gerente", "Gestão operacional dos projetos sob sua responsabilidade e consulta de pastas/permissões; não inclui nem revoga usuários físicos"], ["Patrocinador", "Acompanhamento executivo e consulta dos projetos, mapas e relatórios autorizados"], ["Auditor", "Consulta de projetos, pastas, permissões e trilha de auditoria; sem alteração operacional"], ["Solicitante", "Não possui acesso ao SIGAC; utiliza somente a pasta física do servidor conforme autorização"]], [38 * mm, 140 * mm], s),
        P("O CAV4 autentica e fornece a identidade corporativa. O perfil funcional é carregado do cadastro local no PostgreSQL Aurora pelo e-mail autenticado; grupos e claims do CAV4 não substituem o perfil local. Usuários fixos no seed são apenas bootstrap técnico.", s["Callout"]),
        ...
        P("Entidades principais: users, profiles, modules, permissions, profile_permissions, profile_modules, projects, project_members, folders, access_requests, activity_logs e catálogos. A tabela permission_matrix foi removida; a autorização permanece normalizada nas relações de perfil, módulo e permissão.", s["Body"]),
