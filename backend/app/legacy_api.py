...
async def share(fid:str,x:ShareInput,request:Request):
    await require(request,('admin','gerente')); p=await db();
...
async def user_role(uid:str,x:RolePatch,request:Request):
    await require(request,('admin',)); p=await db(); r=await p.fetchrow('update app_users set role=$1 where id=$2 returning *',x.role,uid)
