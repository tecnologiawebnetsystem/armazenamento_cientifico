...
export function ActivityLogTable() {
  ...
  const update = (key: string, value: string) => { const next = new URLSearchParams(searchParams.toString()); if (value) next.set(key, value); else next.delete(key); if (key !== "page") next.set("page", "1"); router.push(`${pathname}?${next}`) }
  ...
  return <Card className="overflow-hidden border-petrobras-blue/15 bg-gradient-to-br from-background via-background to-petrobras-blue/5 shadow-[0_16px_45px_-28px_rgba(0,88,140,0.45)]">
    <CardHeader className="gap-5 border-b border-petrobras-blue/10 bg-gradient-to-r from-petrobras-green/5 via-background to-petrobras-yellow/10 pb-6"><div className="flex flex-wrap items-start justify-between gap-4"><div className="flex items-start gap-3"><div className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-petrobras-blue/10 text-petrobras-blue shadow-sm"><HistoryIcon className="size-5" aria-hidden="true" /></div><div><CardTitle className="text-xl">Trilha de auditoria</CardTitle><CardDescription className="mt-1">{pagination.total} eventos no escopo autorizado. {isValidating ? "Atualizando…" : ""}</CardDescription></div></div><div className="flex gap-2"><ExportButton onClick={() => setExportOpen(true)} /></div></div>
      <div className="rounded-xl border border-petrobras-blue/10 bg-background/80 p-3 shadow-sm"><div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4"><div className="relative lg:col-span-2"><SearchIcon className="absolute left-3 top-1/2 -translate-y-1/2 text-petrobras-blue" /><Input className="border-petrobras-blue/15 bg-background pl-9 shadow-none focus-visible:ring-petrobras-blue/30" placeholder="Buscar usuário, projeto, ação ou correlationId" value={filters.q} onChange={(e) => update("q", e.target.value)} /></div><select aria-label="Filtrar ação" className="h-9 rounded-md border border-petrobras-green/20 bg-background px-3 text-sm shadow-none" value={filters.acao} onChange={(e) => update("acao", e.target.value)}><option value="">Todas as ações</option>{options.actions.map((item) => <option key={item}>{item}</option>)}</select><select aria-label="Filtrar resultado" className="h-9 rounded-md border border-petrobras-yellow/30 bg-background px-3 text-sm shadow-none" value={filters.resultado} onChange={(e) => update("resultado", e.target.value)}><option value="">Todos os resultados</option><option value="sucesso">Sucesso</option><option value="erro">Erro</option></select><Input aria-label="Data inicial" className="border-petrobras-teal/20 bg-background shadow-none" type="date" value={filters.de} onChange={(e) => update("de", e.target.value)} /><Input aria-label="Data final" className="border-petrobras-teal/20 bg-background shadow-none" type="date" value={filters.ate} onChange={(e) => update("ate", e.target.value)} /></div></div></CardHeader>
    <CardContent className="p-0">{isLoading ? <div className="flex flex-col gap-2 py-4">{Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className="h-14 rounded-lg" />)}</div> : !logs.length ? <Empty><EmptyHeader><EmptyMedia variant="icon"><HistoryIcon /></EmptyMedia><EmptyTitle>Nenhum evento encontrado</EmptyTitle><EmptyDescription>Ajuste os filtros para consultar outro período ou ação.</EmptyDescription></EmptyHeader></Empty> : <><Table><TableHeader><TableRow><TableHead>Data</TableHead><TableHead>Usuário</TableHead><TableHead>Ação</TableHead><TableHead>Projeto</TableHead><TableHead>Resultado</TableHead><TableHead>correlationId</TableHead><TableHead className="text-right">Detalhes</TableHead></TableRow></TableHeader><TableBody>{logs.map((log) => <TableRow key={log.id}><TableCell className="whitespace-nowrap text-xs">{formatAuditDate(log.criadoEm)}</TableCell><TableCell><div className="font-medium">{display(log.user?.nome)}</div><div className="text-xs text-muted-foreground">{display(log.user?.email)}</div></TableCell><TableCell><Badge variant="outline">{display(log.acao)}</Badge><div className="mt-1 text-xs text-muted-foreground">{display(log.entidade)}</div></TableCell><TableCell><div className="font-medium">{display(log.projetoNome)}</div><div className="font-mono text-xs text-muted-foreground">{display(log.projetoId)}</div></TableCell><TableCell>{log.resultado === "erro" ? <Badge variant="destructive"><AlertCircleIcon data-icon="inline-start" />Erro</Badge> : <Badge variant="secondary"><CheckCircle2Icon data-icon="inline-start" />Sucesso</Badge>}</TableCell><TableCell className="max-w-36 truncate font-mono text-xs">{log.correlationId ?? "—"}</TableCell><TableCell className="text-right"><Button variant="ghost" size="sm" onClick={() => setSelected(log)}>Ver evento</Button></TableCell></TableRow>)}</TableBody></Table><div className="flex items-center justify-between border-t pt-4 text-sm text-muted-foreground"><span>Página {pagination.page} de {Math.max(pagination.totalPages, 1)}</span><div className="flex gap-2"><Button variant="outline" size="sm" disabled={pagination.page <= 1} onClick={() => update("page", String(pagination.page - 1))}><ChevronLeftIcon data-icon="inline-start" />Anterior</Button><Button variant="outline" size="sm" disabled={pagination.page >= pagination.totalPages} onClick={() => update("page", String(pagination.page + 1))}>Próxima<ChevronRightIcon data-icon="inline-end" /></Button></div></div></>}</CardContent>
