...
import { BriefcaseBusinessIcon, ChevronDownIcon, FileTextIcon, LogOutIcon, MapPinIcon, Settings2Icon, Share2Icon, UserRoundIcon } from "lucide-react"
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" sideOffset={10} className="w-[min(22rem,calc(100vw-1.5rem))] overflow-hidden rounded-3xl border-border/70 bg-card p-0 text-card-foreground shadow-xl" style={profileStyle}>
        ...
        <div className="px-2 pb-2">
          <DropdownMenuItem onClick={() => router.push("/projetos")} className="h-12 cursor-pointer gap-3 rounded-xl px-4 text-sm"><BriefcaseBusinessIcon />Meus projetos</DropdownMenuItem>
          <DropdownMenuItem onClick={() => router.push("/pesquisas")} className="h-12 cursor-pointer gap-3 rounded-xl px-4 text-sm"><FileTextIcon />Pesquisas e consultas</DropdownMenuItem>
          <DropdownMenuItem onClick={() => router.push("/projetos")} className="h-12 cursor-pointer gap-3 rounded-xl px-4 text-sm"><Share2Icon />Compartilhar acesso</DropdownMenuItem>
          <DropdownMenuItem onClick={() => router.push("/dashboard")} className="h-12 cursor-pointer gap-3 rounded-xl px-4 text-sm"><Settings2Icon />Painel de controle</DropdownMenuItem>
        </div>
        <DropdownMenuSeparator className="m-0" />
