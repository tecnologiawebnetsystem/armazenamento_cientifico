...
const content: Record<SectionId, Page> = {
  ...
  database: { eyebrow: "Persistência", title: "Banco de dados", description: "A persistência é orientada por models SQLAlchemy e migrations Alembic.", blocks: [
    ...
    { title: "Nomes atuais", text: "As tabelas principais são perfis, permissoes, perfil_permissoes, modulos, perfil_modulos, status_projetos, tipos_projetos, configuracoes_sistema, tipos_relatorios, menus, users, projects, project_members, files, file_shares e activity_logs. users.perfil_id referencia perfis.id. Relações usam chaves estrangeiras e índices para consultas de sessão, projeto e auditoria." },
  ] },
  ...
  api: { eyebrow: "Contrato OpenAPI", title: "API e endpoints usados", description: "Lista consolidada das rotas públicas da aplicação. A fonte viva permanece em /docs e /openapi.json.", blocks: [
    ...
    { title: "Usuários e perfis", text: "GET /api/users; PATCH /api/users/{id} com `perfilId`; GET /api/perfis. O cadastro e a edição usam o ID fixo do perfil (`ADM`, `GER`, `AUD`, `PAT`, `PAR`, `VIS`, `GES`). Uso: administração, login e autorização." },
